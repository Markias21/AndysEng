...
test("buildReport: 헤더와 총 문장 수, 학습자를 담는다", () => {
  const md = buildReport(
    { conversation: [{ score: 90, sentence: "I like coffee." }] },
    { dateLabel: "2026-07-16 14:30", userName: "류건" }
  );
  assert.match(md, /# AndysEng 학습 리포트 — 2026-07-16 14:30/);
  assert.match(md, /학습자: 류건/);
...
test("buildReport: 기록이 없는 섹션은 생략한다", () => {
  const md = buildReport(
    ...
    { dateLabel: "2026-07-16 09:00", userName: "류건" }
  );
...
test("buildReport: 글쓰기 피드백을 펼쳐 담는다", () => {
  const md = buildReport(
    ...
    { dateLabel: "2026-07-16 20:00", userName: "류건" }
  );
